...
% A typeclass with $n$ parameters builds an $n$-tuple relation on types.
...
As shown in \pref{fig:add-tc-fd}, !Add m n p! typeclass relates the type parameters such that
...
using the rules shown in \pref{fig:tcfd-opsem}.We define $\manystepsto {\bullet} {\bullet}$ as a transitive closure of
...
The term typing rules are standard and we skip them due to space constraints, the interesting one \trule{t-cast}.
...
that type equality coercions can appear only in $\Preds$. We do not have the rule \trule{v-tfctr} in this system
