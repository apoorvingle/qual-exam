...
% that states equality between types. 
...
in \pref{sec:tf-closed} and  constraint type families\cite{morris_typefamilies_2017}
...
between the three systems while pointing towards some open questions in \pref{sec:conclusion}.
...
% being the negative of the integer, we can define the instances !Semigroup Int! and !Group Int!
...
% In this case, to define an instance for !Group Int! we would first need to define an instance of !Semigroup Int!.
...
The rest are very similar to \CLTF. The rule \trule{t-coabs} abstracts
over coercion variable while \trule{t-coapp} applies a coercion
...
The rule \trule{t-assum} is a special rule that says we are allowed to assume arbitrary applications
...
\subsubsection{Type reduction}
The type reduction relation is given using two rules \trule{qty-red-top} and \trule{qty-red}. The
...
Selected rules for term evalutaion are shown in \pref{fig:tf-constrained-opsem}.
...
the coercion application $\Co$. The rule \trule{t-qres} is the rule that illuminates the evalutation
for total type family constructors. The clause $\FamCtrs(\many\tau) \Downarrow q$ does what? \AI{start here}
